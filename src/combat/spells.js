...
export const SPELLS = [
  ...
  { id: 'frostlance', slot: 3, name: 'Frost Lance',kind: 'projectile', element: 'frost',     color: 0x8fd4ff, mana: 14, cd: 0.85, range: 18,   damage: 24, vfx: 'ice_ball',     speed: 22, pierce: true, telegraphSec: 0.2, meshPath: 'models/vfx/rocks/magic-rock-2.glb' },
...
export const CLASS_LOADOUTS = {
  ...
  raider:   ['cleave', 'void_dash', 'thunder', 'holy_nova', 'fireball', 'frostlance'],
  mage:     ['thunder', 'fireball', 'frostlance', 'holy_nova', 'void_dash', 'cleave'],
  priest:   ['holy_nova', 'thunder', 'void_dash', 'frostlance', 'fireball', 'cleave'],
  ranger:   ['frostlance', 'cleave', 'void_dash', 'fireball', 'thunder', 'holy_nova'],
  thief:    ['cleave', 'void_dash', 'frostlance', 'holy_nova', 'thunder', 'fireball'],
  worge:    WORGE_WEAPONS.tome.slots,
  verduror: WORGE_WEAPONS.nature.slots,
};
...
export function loadoutFor(classId, worgeWeapon = 'tome') {
  if (classId === 'worge' || classId === 'verduror') {
    const alias = { '1h_tome': 'tome', nature_staff: 'nature', arcane_staff: 'arcane', nature: 'nature', tome: 'tome', arcane: 'arcane' };
    const key = alias[worgeWeapon] || (classId === 'verduror' ? 'nature' : 'tome');
    const pack = WORGE_WEAPONS[key] || WORGE_WEAPONS.tome;
    return pack.slots.map(spellById);
  }
